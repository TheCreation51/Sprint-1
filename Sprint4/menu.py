...
class IceStorms: 
    icecreamflavors = {"Mint Chocolate Chip": 4.00, "Chocolate": 3.00, "Vanilla Bean": 3.00, "Banana": 3.50, "Butter Pecan": 3.50, "Smore": 4.00}
    toppings = {"Cherry": 0.00, "Whipped Cream": 0.00, "Caramel Sauce": 0.50, "Chocolate Sauce": 0.50, "Storios": 1.00, "Dig Dogs": 1.00, "T&Ts": 1.00, "Cookie Dough": 1.00, "Pecans": 0.50}
    sizes = {"Small": 1.50, "Medium": 1.75, "Large": 2.05, "Mega": 2.15}
    ...
    def set_food_item(self, flavor):
        flavor = flavor.title()
        if flavor in self.icecreamflavors:
            self.selected_flavor = flavor
        else:
            raise ValueError(f"Invalid ice cream flavor. Valid options are: {', '.join(self.icecreamflavors.keys())}")
    ...
    def add_topping(self, topping):
        ...
        if topping in self.toppings:
            raise ValueError(f"The topping {topping} has already been added.")
        if topping in IceStorms.toppings:
            self.toppings.append(topping)
        else:
            raise ValueError(f"Invalid topping. Valid options are: {', '.join(IceStorms.toppings.keys())}")

    def calculate_price(self):
        if not self.selected_flavor:
            raise ValueError("Ice cream flavor must be set before calculating price.")
        if not self.size:
            raise ValueError("Size must be set before calculating price.")
        base_price = self.icecreamflavors[self.selected_flavor]
        size_price = self.sizes[self.size]
        toppings_cost = sum(IceStorms.toppings[topping] for topping in self.toppings)
        return base_price + size_price + toppings_cost

    def get_selected_flavor(self):
        return self.selected_flavor
...
class Order: 
    ...
    def get_receipt(self):
        ...
        for item in self._items:
            ...
            elif isinstance(item, IceStorms):
                ice_storm_count += 1
                item_price = item.calculate_price()
                receipt.append(
                    f"Ice Storm {ice_storm_count}: Flavor - {item.get_selected_flavor()}, Size - {item.get_size()}, "
                    f"Toppings - {', '.join(item.get_toppings())}, Price - ${item_price:.2f}"
                )
            total += item_price
    ...
    def add_item(self, item):
        if isinstance(item, (Drink, Food, IceStorms)):
            self._items.append(item)
        else:
            raise TypeError("Only Drink, Food, or IceStorms objects can be added to the order.")
    ...
    for item_data in order_data:
        if item_data.get("type") == "Drink":
            ...
            try:
                ...
                return {"error": str(e)}
        elif item_data.get("type") == "IceStorms":
            ice_storm = IceStorms()
            try:
                ice_storm.set_food_item(item_data["flavor"])
                ice_storm.set_size(item_data["size"])
                for topping in item_data.get("toppings", []):
                    ice_storm.add_topping(topping)
...
order_data = [
    ...
    {"type": "IceStorms", "flavor": "Mint Chocolate Chip", "size": "Large", "toppings": ["Cherry", "Cookie Dough", "Pecans"]},
]
