...
class Order: 
    ...
    def get_receipt(self):
        receipt = []
        for index, drink in enumerate(self_items, start=1):
...
if __name__ == "__main__":
    ...
    while True: 
        ...
        base = input("Please select a base: ").strip().lower()
        try:
            drink.set_base(base)
        ...
        print(f"Available flavors: {', '.join(Drink.flavors)}")
        while True:
            flavor = input("Please select a flavor (or Enter to finish): ").strip().lower()
            if not flavor:
                break
            try:
